...
async def get_article(db: AsyncSession, article_id: UUID) -> NewsArticle:
    article = await db.scalar(select(NewsArticle).options(selectinload(NewsArticle.categories)).where(NewsArticle.id == article_id, NewsArticle.deleted_at.is_(None)))
    if not article:
        raise AppError('article_not_found', 'News article not found.', 404)
    return article


async def _set_categories(db: AsyncSession, article: NewsArticle, ids: list[UUID]):
    if not ids:
        article.categories = []
        return
    cats = (await db.scalars(select(NewsCategory).where(NewsCategory.id.in_(ids)))).all()
    if len(cats) != len(set(ids)):
        raise AppError('invalid_categories', 'One or more categories do not exist.', 400)
    article.categories = list(cats)
...
async def create_article(db: AsyncSession, payload: NewsCreate, actor_id: UUID) -> NewsArticle:
    ...
    article = NewsArticle(
        slug=payload.slug, title=payload.title, excerpt=payload.excerpt, body=payload.body,
        hero_image_url=payload.hero_image_url, hero_image_alt=payload.hero_image_alt,
        seo_title=payload.seo_title, meta_description=payload.meta_description, og_image_url=payload.og_image_url,
        author_id=actor_id, created_by=actor_id, updated_by=actor_id,
    ...
    db.add(article)
    await db.flush()
    await _set_categories(db, article, payload.category_ids)
...
async def update_article(db: AsyncSession, article_id: UUID, payload: NewsUpdate, actor_id: UUID) -> NewsArticle:
    ...
    if 'slug' in data and data['slug'] != article.slug:
        if await db.scalar(select(NewsArticle.id).where(NewsArticle.slug == data['slug'], NewsArticle.id != article.id)):
            raise AppError('slug_exists', 'An article with this slug already exists.', 409)
    for key, value in data.items():
        setattr(article, key, value)
    if category_ids is not None:
        await _set_categories(db, article, category_ids)
